...
class Robot:
    ...
    def steerTo(self, goal_pose, publisher, twist_msg):
        ...
        speedConstant = 0.01
        angularVel = (angularDistance) * speedConstant
        ...
        while (abs(angularDistance) > angle_tolerance):
            #print("Angular Distance: ", abs(angularDistance))
            ...
            if (abs(angularDistance) < angle_tolerance+0.001):
            	print("Almost there!, Current angular distance is: ", abs(angularDistance), " Current angular velocity: ", angularVel)
    ...
    def moveGoal(self, goal_pose, distance_tolerance):
        ...
        pub.publish(vel_msg)
        rate.sleep()
        print("I did it! I arrived at: (", goal_pose.x, ", ", goal_pose.y,")")

    def createPose(self, x,y, theta=0):
        returnPose = Pose()
        returnPose.x = x
        returnPose.y = y
        returnPose.theta = theta
        return returnPose

    def drive_square(self, side_length):
        # if rate in rotate is one it will become a triangle
        
        #rate = rospy.Rate(10)
        
        for i in range(4):
            print("Driving side "+str(i+1)+"\n")
            move(side_length, isForward=True)
            print("Rotating \n")
            rotate(degrees2radians(90.0), isClockwise=False)
            
            #rate.sleep()
            
            # points the wrong way at the end
            # rate.sleep(0)
